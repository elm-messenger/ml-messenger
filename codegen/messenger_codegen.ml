...
let module_name_of_msg msg =
  match List.rev (String.split_on_char '.' msg) with
  | "msg" :: module_name :: _ when module_name <> "" -> module_name
  | _ -> die "invalid message path %s (expected Module.msg)" msg
...
let find_user_msg_file root msg =
  let module_name = module_name_of_msg msg in
  let expected = String.uncapitalize_ascii module_name ^ ".ml" in
  let module_parts =
    match List.rev (String.split_on_char '.' msg) with
    | "msg" :: _module_name :: rev_dirs -> List.rev rev_dirs
    | _ -> []
  in
  let candidate =
    List.fold_left
  ...
  if Sys.file_exists candidate then candidate
    ...
    match matches with
    | [ path ] -> path
    | [] -> die "cannot find source file for user message %s" msg
    | _ ->
...
let base_ml root (scenes : scene list) (groups : group list) =
  ...
  let user_messages =
    all_groups
    |> List.concat_map (fun (_namespace, components) -> components)
    |> List.filter_map (fun c ->
           if String.equal c.kind "user" then
             let msg = require_component_msg c in
             Some (module_name_of_msg msg, find_user_msg_file root msg)
           else None)
    |> List.fold_left
         (fun acc (module_name, source_file) ->
           match List.assoc_opt module_name acc with
           | None -> (module_name, source_file) :: acc
           | Some existing when String.equal existing source_file -> acc
           | Some _ ->
               die "message module %s is used by multiple source files"
                 module_name)
         []
    |> List.sort (fun (a, _) (b, _) -> String.compare a b)
  in
  if user_messages <> [] then (
    Buffer.add_string b "module User_messages = struct\n";
    List.iter
      (fun (module_name, source_file) ->
        Buffer.add_string b
          (Printf.sprintf "  module %s = struct\n" module_name);
        Buffer.add_string b (read_file source_file);
        Buffer.add_string b "\n  end\n")
      user_messages;
  ...
  List.iter
    (fun (namespace, components) ->
      ...
      (* Keep Mgl_base independent from the Scenes aggregate. *)
      let group_user_messages =
        components
        |> List.filter_map (fun c ->
               if String.equal c.kind "user" then
                 let msg = require_component_msg c in
                 Some (module_name_of_msg msg)
               else None)
      ...
      List.iter
        (fun module_name ->
          Buffer.add_string b
            (Printf.sprintf "      module %s = User_messages.%s\n" module_name
               module_name))
        group_user_messages;
      ...
      List.iter
        (fun c ->
          let msg_type =
            ...
            | "user" -> module_name_of_msg (require_component_msg c) ^ ".msg"
