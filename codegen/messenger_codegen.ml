...
let get_project file =
  let tbl = parse_toml file in
  {
    ...
    user_data_type =
      Option.value
        (get_string_opt file tbl [ "main"; "user_data_type" ])
...
let find_msg_file root msg =
  ...
  if Sys.file_exists candidate then candidate
    ...
    | _ ->
        die "message module %s is ambiguous; found multiple %s files" msg
          expected
...
type msg_tree = { source_file : string option; children : msg_tree StringMap.t }
...
let rec add_msg_source full_path parts source_file tree =
  ...
  | part :: rest ->
      let child =
        Option.value
          (StringMap.find_opt part tree.children)
...
let base_ml root (scenes : scene list) (groups : group list) =
  ...
  let portable_components =
    List.filter
      (fun (c : component) -> String.equal c.kind "portable")
      all_components
  in
  ...
  let message_tree =
    List.fold_left
      (fun tree msg ->
        add_msg_source msg (msg_module_parts msg) (find_msg_file root msg) tree)
...
let all_ml project (scenes : scene list) =
  ...
  Buffer.add_string b "(* @generated by messenger_codegen; do not edit *)\n\n";
  Buffer.add_string b
    (Printf.sprintf "type user_data = %s\n" project.user_data_type);
...
let source_project_root project_file fallback =
  ...
  | Some source_root -> (
      let build_prefix =
        Filename.concat source_root "_build" ^ Filename.dir_sep
      in
...
let () =
  ...
  ensure_dir out_dir;
  write_file
    (Filename.concat out_dir "mgl_base.ml")
