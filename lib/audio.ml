...
let new_audio_channel (runtime : Internal.runtime) =
  runtime.audio_repo.playing |> List.map (fun (p : Internal.playing_audio) -> p.channel) |> List.fold_left max (-1) |> ( + ) 1

let audio_duration (runtime : Internal.runtime) name =
  match Hashtbl.find_opt runtime.audio_repo.audio name with None -> None | Some src -> Some (Regl_audio.length src)

let config_of_option = function
  | Audio_base.A_once common ->
      let c = Regl_audio.default_config in
      let common = Option.value common ~default:Audio_base.default_common_option in
      Some { c with playback_rate = common.rate; start_at = common.start; loop = None }
  | A_loop (common, loop) ->
      let c = Regl_audio.default_config in
      let common = Option.value common ~default:Audio_base.default_common_option in
...
let play_audio (repo : Internal.audio_repo) channel name opt now =
  ...
  | None -> ()
  | Some source -> repo.playing <- { Internal.channel; name; source; opt; start_time = now } :: repo.playing
...
let update_audio (_repo : Internal.audio_repo) (_target : Audio_base.audio_target) (_f : Regl_audio.audio -> Regl_audio.audio) =
  ()
