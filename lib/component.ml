...
type ('cdata, 'userdata, 'msg, 'data, 'bdata) component_init =
  Internal.runtime -> ('cdata, 'userdata) Base.env -> 'msg -> 'data * 'bdata

type ('cdata, 'data, 'userdata, 'scenemsg, 'tar, 'msg, 'bdata) component_update =
  Internal.runtime ->
  ('cdata, 'userdata) Base.env ->
  Regl_proto.regl_event ->
  'data ->
...
type ('cdata, 'userdata, 'data, 'bdata) component_view =
  Internal.runtime ->
  ('cdata, 'userdata) Base.env ->
  'data ->
  'bdata ->
...
type ('cdata, 'userdata, 'tar, 'msg, 'bdata, 'scenemsg) level_component_storage =
  Internal.runtime ->
  ('cdata, 'userdata) Base.env ->
  ('cdata, 'userdata, 'tar, 'msg, 'bdata, 'scenemsg) abstract_component

type ('cdata, 'userdata, 'tar, 'msg, 'bdata, 'scenemsg) component_storage =
  'msg ->
  ('cdata, 'userdata, 'tar, 'msg, 'bdata, 'scenemsg) level_component_storage

let gen_component concomp = General_model.abstract concomp

let update_components runtime env evt comps =
  Recursion.update_objects runtime env evt comps

let update_components_with_block runtime env evt block comps =
  if block then (comps, [], (env, true))
  else update_components runtime env evt comps

let update_components_with_target runtime env msgs comps =
  Recursion.update_objects_with_target runtime env msgs comps
