...
let init () _runtime _env _msg = ((), { Scene.dead = false; post_processor = Fun.id })
...
let view runtime env () _bdata =
  let virtual_height = env.Base.global_data.camera.Ml_regl_core.Regl_common.y *. 2. in
  let spinner =
    List.init 8 (fun i ->
        let fi = float_of_int i in
        let x = 15. *. cos ((Float.pi /. 4.) *. fi) in
        let y = 15. *. sin ((Float.pi /. 4.) *. fi) in
        let radius = 2. +. sin ((Base.get_global_start_time runtime *. 0.005) +. (2. *. Float.pi *. fi /. 8.)) in
        Regl_builtin_programs.circle (30. +. x, virtual_height -. 30. +. y) radius Color.white)
  in
