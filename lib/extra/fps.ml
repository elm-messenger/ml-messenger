...
type data = { last_times : float list; fps : float; size : float; font : string }

let trim_to_last_ten xs =
  if List.length xs >= 10 then match xs with _ :: tl -> tl | [] -> [] else xs

let init opt _runtime _env _msg =
  ({ last_times = []; fps = 0.; size = opt.font_size; font = opt.font }, { Scene.dead = false; post_processor = Fun.id })
...
let update _runtime env evnt data bdata =
  ...
  | Regl_proto.UpdateTick _ ->
      ...
      let fps = if sum <= 0. then 0. else float_of_int (List.length last_times) /. sum *. 1000. in
...
let view _runtime _env data _bdata =
  Regl_builtin_programs.textbox (0., 0.) data.size ("FPS: " ^ string_of_int (int_of_float data.fps)) data.font (Color.rgba 0. 0. 0. 0.5)
