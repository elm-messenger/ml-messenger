...
type ('cdata, 'data, 'userdata, 'tar, 'msg, 'bdata, 'scenemsg) concrete_iu_component = {
  init : ('cdata, 'userdata, 'msg, 'data, 'bdata) Component.component_init;
  init_update : ('cdata, 'data, 'userdata, 'scenemsg, 'tar, 'msg, 'bdata) Component.component_update;
  update : ('cdata, 'data, 'userdata, 'scenemsg, 'tar, 'msg, 'bdata) Component.component_update;
  updaterec : ('cdata, 'data, 'userdata, 'scenemsg, 'tar, 'msg, 'bdata) Component.component_updaterec;
  view : ('cdata, 'userdata, 'data, 'bdata) Component.component_view;
  matcher : ('data, 'bdata, 'tar) Component.component_matcher;
}

let res_map (((data, bdata), msgs, envres) : ('data * 'bdata) * 'msgs * 'envres) inited =
...
let to_concrete_user_component comp =
  ...
  in
  let updaterec runtime env msg (data, inited) bdata = res_map (comp.updaterec runtime env msg data bdata) inited in
...
let gen_iu_component comp = Component.gen_component (to_concrete_user_component comp)
