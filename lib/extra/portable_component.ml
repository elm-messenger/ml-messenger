...
type ('userdata, 'msg, 'data) portable_component_init =
  Internal.runtime -> (unit, 'userdata) Base.env -> 'msg -> 'data
...
type ('userdata, 'data) portable_component_view =
  Internal.runtime ->
  (unit, 'userdata) Base.env ->
  'data ->
...
type ('data, 'userdata, 'tar, 'msg, 'scenemsg) concrete_portable_component = {
  ...
  updaterec :
    ('data, 'userdata, 'tar, 'msg, 'scenemsg) portable_component_updaterec;
...
let decode_msg msgcodec tarcodec = function
  | General_model.Parent (OtherMsg othermsg) ->
      General_model.Parent (OtherMsg (msgcodec.decode othermsg))
...
let translate_portable_component pcomp tarcodec msgcodec empty_base_data zindex
    =
  let init runtime env gmsg =
    ( pcomp.init runtime (Base.remove_common_data env) (msgcodec.encode gmsg),
      empty_base_data )
  in
  let update runtime env evt data base_data =
    let res_data, res_msg, (res_env, res_block) =
      pcomp.update runtime (Base.remove_common_data env) evt data
    in
    ( (res_data, base_data),
      List.map (decode_msg msgcodec tarcodec) res_msg,
      (Base.add_common_data env.common_data res_env, res_block) )
  in
  let updaterec runtime env gmsg data base_data =
    let res_data, res_msg, res_env =
      pcomp.updaterec runtime
        (Base.remove_common_data env)
        (msgcodec.encode gmsg) data
    in
    ( (res_data, base_data),
      List.map (decode_msg msgcodec tarcodec) res_msg,
      Base.add_common_data env.common_data res_env )
  in
  let view runtime env data _ =
    (pcomp.view runtime (Base.remove_common_data env) data, zindex)
  in
...
let gen_portable_component conpcomp tcodec mcodec empty_base_data zindex =
  Component.gen_component
    (translate_portable_component conpcomp tcodec mcodec empty_base_data zindex)
