...
type ('tar, 'msg, 'sommsg) msg = Parent of ('msg, 'sommsg) msg_base | Other of 'tar * 'msg

let filter_som xs =
  List.filter_map (function SOMMsg som -> Some som | OtherMsg _ -> None) xs

type ('data, 'envro, 'env, 'event, 'tar, 'msg, 'ren, 'bdata, 'sommsg) concrete_general_model = {
...
type ('envro, 'env, 'event, 'tar, 'msg, 'ren, 'bdata, 'sommsg) abstract_general_model =
  | Roll of ('envro, 'env, 'event, 'tar, 'msg, 'ren, 'bdata, 'sommsg) unrolled_abstract_general_model

and ('envro, 'env, 'event, 'tar, 'msg, 'ren, 'bdata, 'sommsg) unrolled_abstract_general_model = {
  update :
    'envro ->
    'env ->
    'event ->
    ('envro, 'env, 'event, 'tar, 'msg, 'ren, 'bdata, 'sommsg) abstract_general_model
  ...
  base_data : 'bdata;
  update_base_data :
    'bdata -> ('envro, 'env, 'event, 'tar, 'msg, 'ren, 'bdata, 'sommsg) abstract_general_model;
}

let unroll (Roll un) = un

let abstract (type data envro env event tar msg ren bdata sommsg)
    (conmodel : (data, envro, env, event, tar, msg, ren, bdata, sommsg) concrete_general_model) init_msg init_envro init_env =
  let rec abstract_rec data base =
    let update envro env event =
      let (new_d, new_bd), new_m, new_e = conmodel.update envro env event data base in
      (abstract_rec new_d new_bd, new_m, new_e)
    in
    let updaterec envro env msg =
      let (new_d, new_bd), new_m, new_e = conmodel.updaterec envro env msg data base in
      (abstract_rec new_d new_bd, new_m, new_e)
    in
...
let view_model_list envro env models = List.map (fun model -> (unroll model).view envro env) models
