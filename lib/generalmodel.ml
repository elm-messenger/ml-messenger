...
let abstract 
    (conmodel : ('data, 'env, 'event, 'tar, 'msg, 'ren, 'bdata, 'sommsg) concrete_general_model)
    (init_msg : 'msg) 
    (init_env : 'env) : ('env, 'event, 'tar, 'msg, 'ren, 'bdata, 'sommsg) abstract_general_model =
  
  let rec abstract_rec (data : 'data) (base : 'bdata) : ('env, 'event, 'tar, 'msg, 'ren, 'bdata, 'sommsg) abstract_general_model =
    ...
    let updaterecs env msg =
      let ((new_d, new_bd), new_m, new_e) = conmodel.updaterec env msg data base in
      (abstract_rec new_d new_bd, new_m, new_e)
    in
    
    let views env = conmodel.view env data base in
    let matchers = conmodel.matcher data base in
    let base_datas = base in
...
(* View model list *)
let view_model_list 
    (env : ('common, 'userdata) env) 
    (models : (('common, 'userdata) env, user_event, 'tar, 'msg, 'ren, 'bdata, 'sommsg) abstract_general_model list) : 'ren list =
...
let update_result_remap 
    (f : (('tar, 'msg, 'sommsg) msg list * ('env * bool)) -> (('tar, 'msg, 'sommsg) msg list * ('env * bool)))
    (model : ('env, 'event, 'tar, 'msg, 'ren, 'bdata, 'sommsg) abstract_general_model) : 
    ('env, 'event, 'tar, 'msg, 'ren, 'bdata, 'sommsg) abstract_general_model =
...
let updaterec_result_remap 
    (f : (('tar, 'msg, 'sommsg) msg list * 'env) -> (('tar, 'msg, 'sommsg) msg list * 'env))
    (model : ('env, 'event, 'tar, 'msg, 'ren, 'bdata, 'sommsg) abstract_general_model) : 
    ('env, 'event, 'tar, 'msg, 'ren, 'bdata, 'sommsg) abstract_general_model =
