...
let filter_unfinished_msg msgs =
  List.filter_map (function
    | Parent _ -> None
    | Other msg -> Some msg) msgs

(* Update one object in the list *)
let rec update_one last_env evt objs last_objs last_msg_unfinished last_msg_finished =
  match objs with
  | ele :: rest_objs ->
    let (new_obj, new_msg, (new_env, block)) = (unroll ele).update last_env evt in
    ...
    if block then
      (rev rest_objs @ [new_obj] @ last_objs, 
       (last_msg_unfinished @ unfinished_msg, last_msg_finished @ finished_msg), 
       (last_env, block))
    else
    ...
    let (new_objs, (new_unfinished_msg, new_finished_msg), new_env) =
      List.fold_left 
        (fun (last_objs, (last_msg_unfinished, last_msg_finished), last_env) ele ->
          let msg_matched = 
            List.filter_map (fun (tar, msg) ->
              if (unroll ele).matcher tar then Some msg else None) unfinished_msg
          in
          ...
          if msg_matched = [] then
            ...
            let (new_obj, (new_msg_unfinished, new_msg_finished), new_env2) =
              List.fold_left
                (fun (last_obj2, (last_msg_unfinished2, last_msg_finished2), last_env2) msg ->
                  let (new_ele, new_msgs, new_env3) = (unroll last_obj2).updaterec last_env2 msg in
                  let finished_msgs = filter_finished_msg new_msgs in
                  let unfinished_msgs = filter_unfinished_msg new_msgs in
                  (new_ele, 
                   (last_msg_unfinished2 @ unfinished_msgs, last_msg_finished2 @ finished_msgs), 
                   new_env3))
                (ele, ([], []), last_env)
                msg_matched
            in
            (last_objs @ [new_obj], 
             (last_msg_unfinished @ new_msg_unfinished, last_msg_finished @ new_msg_finished), 
             new_env2))
        ([], ([], []), env)
        objs
    in
    update_remain new_env (new_unfinished_msg, finished_msg @ new_finished_msg) new_objs
...
let update_objects_with_target env msgs objs =
  update_remain env (msgs, []) objs

(* Remove all objects by target *)
let remove_objects t xs =
  List.filter (fun x -> not ((unroll x).matcher t)) xs
