...
type ('data, 'envro, 'env, 'event, 'ren, 'scenemsg, 'userdata) concrete_scene = {
  ...
  update : 'envro -> 'env -> 'event -> 'data -> 'data * ('scenemsg, 'userdata) scene_output_msg list * 'env;
  view : 'envro -> 'env -> 'data -> 'ren;
}

and ('envro, 'env, 'event, 'ren, 'scenemsg, 'userdata) abstract_scene =
  | Roll of ('envro, 'env, 'event, 'ren, 'scenemsg, 'userdata) unrolled_abstract_scene
...
and ('scenemsg, 'userdata) scene_output_msg =
  ...
  | SOMTransformAudio of Audio_base.audio_target * (Regl_audio.audio -> Regl_audio.audio)
...
and ('userdata, 'scenemsg) scene_storage =
  'scenemsg option -> Internal.runtime -> (unit, 'userdata) Base.env -> ('userdata, 'scenemsg) m_abstract_scene

and ('userdata, 'scenemsg) all_scenes = (string, ('userdata, 'scenemsg) scene_storage) Hashtbl.t

and ('tar, 'msg, 'scenemsg, 'userdata) m_msg = ('tar, 'msg, ('scenemsg, 'userdata) scene_output_msg) General_model.msg

and ('msg, 'scenemsg, 'userdata) m_msg_base = ('msg, ('scenemsg, 'userdata) scene_output_msg) General_model.msg_base

and ('data, 'common, 'userdata, 'tar, 'msg, 'bdata, 'scenemsg) m_concrete_general_model =
  ( 'data,
    Internal.runtime,
...
and ('userdata, 'scenemsg) m_abstract_scene =
  (Internal.runtime, (unit, 'userdata) Base.env, Regl_proto.regl_event, Regl_common.renderable, 'scenemsg, 'userdata) abstract_scene

and gc_common_data = unit

and gc_base_data = { dead : bool; post_processor : Regl_common.renderable -> Regl_common.renderable }

and gc_msg = string
and gc_target = string

and ('userdata, 'scenemsg) abstract_global_component =
  (('userdata, 'scenemsg) m_abstract_scene, 'userdata, gc_target, gc_msg, gc_base_data, 'scenemsg) m_abstract_general_model
...
and ('data, 'userdata, 'scenemsg) concrete_global_component = {
  init : Internal.runtime -> (('userdata, 'scenemsg) m_abstract_scene, 'userdata) Base.env -> gc_msg -> 'data * gc_base_data;
  update :
    Internal.runtime ->
    (('userdata, 'scenemsg) m_abstract_scene, 'userdata) Base.env ->
    Regl_proto.regl_event ->
    'data ->
    gc_base_data ->
    ('data * gc_base_data) * (gc_target, gc_msg, 'scenemsg, 'userdata) m_msg list * ((('userdata, 'scenemsg) m_abstract_scene, 'userdata) Base.env * bool);
  updaterec :
    Internal.runtime ->
    (('userdata, 'scenemsg) m_abstract_scene, 'userdata) Base.env ->
    gc_msg ->
    'data ->
    gc_base_data ->
    ('data * gc_base_data) * (gc_target, gc_msg, 'scenemsg, 'userdata) m_msg list * (('userdata, 'scenemsg) m_abstract_scene, 'userdata) Base.env;
  view : Internal.runtime -> (('userdata, 'scenemsg) m_abstract_scene, 'userdata) Base.env -> 'data -> gc_base_data -> Regl_common.renderable;
  id : gc_target;
}
...
let abstract (type data envro env event ren scenemsg userdata)
    (conmodel : (data, envro, env, event, ren, scenemsg, userdata) concrete_scene) init_msg init_envro init_env =
...
let empty_scene () : ('userdata, 'scenemsg) m_abstract_scene =
  let rec scene = Roll { update = (fun _ env _ -> (scene, [], env)); view = (fun _ _ -> Ml_regl_core.Regl_common.group [] []) } in
  scene
