...
let builtin_programs = function
  ...
  | BasicShapesBuiltinProgram -> Some [ "textbox"; "triangle"; "circle"; "quad"; "poly" ]
  | AllBuiltinProgram -> None

let add_pending_data (runtime : Internal.runtime) path key =
  let old = Option.value (Hashtbl.find_opt runtime.Internal.pending_data_paths path) ~default:[] in
  Hashtbl.replace runtime.pending_data_paths path (key :: old)

let load_resource_command (runtime : Internal.runtime) key = function
  | Resources.Texture_res (url, opts) -> Some (Regl_proto.load_texture key url opts)
  | Audio_res url ->
...
let maybe_start input model =
  if model.Model.started || model.runtime.loaded_res_num < model.runtime.tot_res_num || model.runtime.startup_failed <> [] then model
  else
    let model = Loader.load_scene_by_name input.config.init_scene input.scenes input.config.init_scene_msg model in
    let env_for_gc = model.env in
    let gcs = List.map (fun gc -> gc model.runtime env_for_gc) input.global_components in
...
let init input () =
  ...
  in
  let resource_cmds = List.filter_map (fun (key, res) -> load_resource_command runtime key res) input.resources in
  let outputs = Regl_proto.start_regl start_config :: Regl_proto.config_regl (ConfigTimeInterval input.config.time_interval) :: resource_cmds in
...
let view input model =
  ...
  if not model.Model.started then Regl_common.group [] []
    ...
    let scene_view = (Scene.unroll model.env.common_data).view model.runtime (Base.remove_common_data model.env) in
    let gc_view = General_model.view_model_list model.runtime model.env model.global_components in
    let scene_with_camera = Camera.apply model.env.global_data.camera (post_process scene_view (Global_component.combine_pp model.global_components)) in
...
let handle_regl_recv input model msg =
  ...
  | REGLFontLoaded name ->
      ...
      r.loaded_res_num <- r.loaded_res_num + 1
  | REGLFontLoadFail name -> r.startup_failed <- ("font:" ^ name) :: r.startup_failed
  | REGLProgramCreated name ->
      r.programs <- Internal.StringSet.add name r.programs;
      r.loaded_res_num <- r.loaded_res_num + 1
  | REGLProgramCreateFail name -> r.startup_failed <- ("program:" ^ name) :: r.startup_failed
  | REGLFileLoaded { path; data } ->
      let keys = Option.value (Hashtbl.find_opt r.pending_data_paths path) ~default:[ path ] in
      List.iter (fun key -> Hashtbl.replace r.config_data key data; r.loaded_res_num <- r.loaded_res_num + 1) keys;
      Hashtbl.remove r.pending_data_paths path
  | REGLFileLoadFailed { path; reason } -> r.startup_failed <- ("file:" ^ path ^ ":" ^ reason) :: r.startup_failed
...
let handle_audio_msg input model msg =
  ...
  | Regl_proto.AudioLoadSuccess { audio_url; source } ->
      let key = Option.value (Hashtbl.find_opt r.pending_audio_urls audio_url) ~default:audio_url in
      Hashtbl.replace r.audio_repo.audio key source;
      r.loaded_res_num <- r.loaded_res_num + 1
  | AudioLoadFailed { audio_url; _ } -> r.startup_failed <- ("audio:" ^ audio_url) :: r.startup_failed
  | AudioContextReady _ -> ());
  (handle_resource_loaded input model, [])
...
let rec handle_som input som model =
  let r = model.Model.runtime in
  match som with
  | Scene.SOMChangeScene (msg, name) -> (Loader.load_scene_by_name name input.scenes msg model |> Model.reset_scene_start_time, [])
  ...
  | SOMSetVolume v ->
      r.volume <- v;
      (model, [])
  | SOMLoadGC gc -> ({ model with global_components = model.global_components @ [ gc r model.env ] }, [])
  | SOMUnloadGC target -> ({ model with global_components = Recursion.remove_objects target model.global_components }, [])
  | SOMCallGC (tar, msg) ->
      let call = (tar, msg) in
      let gc1, som1, env1 = Recursion.update_objects_with_target r model.env [ call ] model.global_components in
      let model1 = { model with global_components = gc1; env = env1 } in
      handle_soms input (General_model.filter_som som1) model1
  | SOMChangeFPS fps -> (model, [ Regl_proto.config_regl (ConfigTimeInterval fps) ])
...
let game_update input evnt model =
  if not model.Model.started then (model, [])
    ...
    let gc2, gcsompre, (env2, block) = Recursion.update_objects model.runtime model.env evnt gc1 in
    ...
    let scenesom, model2 =
      if block then ([], model1)
        ...
        let scene, psom, env = (Scene.unroll env2.common_data).update model.runtime (Base.remove_common_data env2) evnt in
        (psom, { model1 with env = Base.add_common_data scene env })
    in
    let model3 =
      match evnt with Regl_proto.UpdateTick _ -> Model.update_scene_time model2 model.runtime.last_frame_delta | _ -> model2
    in
...
let update_input_state (r : Internal.runtime) = function
  ...
  | MouseDown { button; x; y } ->
      r.pressed_mouse_buttons <- Internal.IntSet.add button r.pressed_mouse_buttons;
      r.mouse_pos <- (x, y)
  | MouseUp { button; x; y } ->
      r.pressed_mouse_buttons <- Internal.IntSet.remove button r.pressed_mouse_buttons;
      r.mouse_pos <- (x, y)
...
let gen_main input = Regl_backend.create_app (fun () -> init input ()) (update input) (view input)
