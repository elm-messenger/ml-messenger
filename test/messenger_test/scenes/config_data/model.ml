...
let scene_con : (_, _, _, _, _, _, _) Scene.concrete_scene =
  {
    ...
    view =
      (fun runtime _env () ->
        let text = Option.value (Base.get_config_data "texts" runtime) ~default:"<missing config data>" in
        Regl_common.group []
          [ Regl_builtin_programs.clear Color.white;
            Regl_builtin_programs.textbox (20., 20.) 28. "ConfigData (Backspace home)" "firacode" Color.black;
            Regl_builtin_programs.textbox (20., 80.) 24. text "firacode" (Color.rgb 0.1 0.2 0.6) ]);
  }
