...
type data = {
  initdata : button_init_data;
  pos : float * float;
  cur_state : button_state;
}
...
let update runtime env evnt data basedata =
  let is_hovered =
    Camera.judge_mouse_rect
      ...
      ~pos:data.pos ~size:data.initdata.button_size
  in
  let next_state =
  ...
  | Regl_proto.MouseUp _ ->
      ( ({ data with cur_state = Normal }, basedata),
        [ General_model.Parent (OtherMsg (ButtonUpdateMsg ButtonReleased)) ],
        (env, false) )
  | MouseDown _ ->
      if is_hovered then
        ( ({ data with cur_state = Pressed }, basedata),
          [ General_model.Parent (OtherMsg (ButtonUpdateMsg ButtonPressed)) ],
          (env, true) )
...
let view _runtime _env data _basedata =
  ...
  ( Regl_common.group []
      [
        Regl_builtin_programs.rect_centered data.initdata.button_center rsize 0.
          data.initdata.button_color;
        Regl_builtin_programs.textbox_centered data.initdata.button_center 30.
          data.initdata.button_content "firacode" Color.black;
      ],
    0 )
